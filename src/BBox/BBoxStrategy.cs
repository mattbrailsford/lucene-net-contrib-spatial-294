...
namespace Lucene.Net.Spatial.BBox
{
    public class BBoxStrategy : SpatialStrategy
    {
        ...
        public AbstractField[] CreateIndexableFields(Rectangle bbox)
        {
            ...
            var f = new Field(field_xdl, bbox.GetCrossesDateLine() ? "T" : "F", Field.Store.NO, Field.Index.NOT_ANALYZED_NO_NORMS);
            f.SetOmitNorms(true);
		...
		Query MakeIntersects(Rectangle bbox)
		{
		    ...
		    var qNotDisjoint = new BooleanQuery(); // {{qHasEnv, BooleanClause.Occur.MUST}, {qDisjoint, BooleanClause.Occur.MUST_NOT}};
		...
		{
		    var bq = new BooleanQuery(); //{{this.MakeXDL(crossedDateLine), BooleanClause.Occur.MUST}, {query, BooleanClause.Occur.MUST}};
