...
namespace Lucene.Net.Spatial.Prefix
{
    ...
    public abstract class PrefixTreeStrategy : SpatialStrategy
    {
        ...
        public AbstractField[] CreateIndexableFields(Shape shape, double distErr)
        {
            ...
            var f = new Field(GetFieldName(), new CellTokenStream(cells.GetEnumerator()));
            f.SetOmitNorms(true);
            f.SetOmitTermFreqAndPositions(true);

            return new AbstractField[] { f };
        }
        ...
        protected class CellTokenStream : TokenStream
        {
            ...
            // Taken from https://lucenenet.apache.org/docs/3.0.3/d8/d6f/_attribute_source_8cs_source.html
            public virtual T AddAttribute<T>() where T : Net.Util.Attribute
            {
                var attClass = typeof(T);
                if (!HasAttribute(attClass))
                {
                    if (!(attClass.IsInterface && typeof(Net.Util.Attribute).IsAssignableFrom(attClass)))
                    {
                    ...
                    AddAttributeImpl(GetAttributeFactory().CreateAttributeInstance(typeof(T)));
                }

                return (T)GetAttribute(attClass);
            }
        }
